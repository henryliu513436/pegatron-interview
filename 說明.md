...
- `report.html` / `report.pdf`：技術報告（由 `report.md` 轉出）。

### 產生技術報告
`report.md` 為報告主體，透過 `build_report.py` 轉為列印用 HTML，再由瀏覽器匯出 PDF：
```bash
pip install markdown          # 建置工具依賴，故未列入 requirements.txt
python3 build_report.py       # 產生 report.html
open report.html              # Cmd+P → 另存為 PDF（A4、邊界設「無」、勾選背景圖形）
```
架構圖原始碼位於 `docs/architecture.mmd`，可貼至 [mermaid.live](https://mermaid.live) 重新匯出 PNG。
...
- **方案設計**：由 Claude Sonnet (claude.ai 免費版) 進行核心 Brainstorming 與技術諮詢。
